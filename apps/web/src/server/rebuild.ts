...
export const rebuildAccount = (accountId: string): void => {
  ...
  const executionInputs: Execution[] = rows.map((row) => ({
    id: row.id, accountId: row.accountId, symbol: row.symbol, side: row.side, quantity: row.quantity,
    price: row.price, fee: row.fee, executedAt: row.executedAt,
    assetClass: (row.assetClass ?? undefined) as Execution["assetClass"], source: row.source,
    importMetadata: row.importMetadataJson ? JSON.parse(row.importMetadataJson) : undefined,
  }));
  const trips = buildRoundTrips(executionInputs, { method: account.profitCalcMethod as ProfitCalcMethod, multipliers: getMultipliers() });
  const existing = db.select({ key: trades.key, fundingFee: trades.fundingFee }).from(trades).where(eq(trades.accountId, accountId)).all();
  const fundingByKey = new Map(existing.map((row) => [row.key, row.fundingFee]));
  ...
  db.transaction((tx) => {
    for (const trip of trips) {
      ...
      > = {
        ...
        fees: trip.fees, netPnl, executionCount: trip.executionCount, executionIdsJson: JSON.stringify(trip.executionIds),
        exitsJson: JSON.stringify(trip.exits), durationMs: trip.durationMs ?? null,
      };
      tx.insert(trades).values({ key: trip.key, ...computed, fundingFee, ...defaultRisk(trip.avgEntry, trip.direction, accountId, trip.symbol, defaults) })
        .onConflictDoUpdate({ target: trades.key, set: computed }).run();
    }
    const vanished = [...obsolete];
    for (let i = 0; i < vanished.length; i += 500) {
      tx.delete(trades).where(and(eq(trades.accountId, accountId), inArray(trades.key, vanished.slice(i, i + 500)))).run();
    }
  });
};
